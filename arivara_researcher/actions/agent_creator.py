...
async def choose_agent(
    ...
    **kwargs
    ...
    # Extract token_tracker from kwargs or researcher if available
...
async def handle_json_error(response):
    # Handle None response (e.g., when API quota is exceeded)
    if response is None:
        print("⚠️ LLM Response is None (likely due to API quota/rate limit). Falling back to Default Agent.")
    ...
    if not isinstance(response, str):
        print(f"⚠️ LLM Response is not a string (type: {type(response)}). Falling back to Default Agent.")
        return "Default Agent", (
            "You are an AI critical thinker research assistant. Your sole purpose is to write well written, "
            "critically acclaimed, objective and structured reports on given text."
        )
...
def extract_json_with_regex(response):
    """Extract JSON from response string using regex."""
