...
class Arivara_researcher:
    ...
    async def conduct_research(self, on_progress=None):
        ...
        if not (self.agent and self.role):
            ...
            kwargs_with_researcher = self.kwargs.copy()
            kwargs_with_researcher["researcher"] = self
    ...
    async def write_report(self, existing_headers: list = [], relevant_written_contents: list = [], ext_context=None, custom_prompt="") -> str:
        ...
        logger.info(f"=== Arivara_researcher.write_report() CALLED for report_type={self.report_type} ===")
        logger.info(f"Websocket status: {self.websocket is not None}, type={type(self.websocket).__name__ if self.websocket else 'None'}")
        logger.info(f"Context length: {len(self.context) if self.context else 0}")
        ...
        logger.info(f"Calling report_generator.write_report() with websocket={self.websocket is not None}")
        ...
        logger.info(f"report_generator.write_report() returned report of length={len(report) if report else 0}")
        
        await self._log_event("research", step="report_completed", details={
            "report_length": len(report)
        })
        
        logger.info(f"=== Arivara_researcher.write_report() COMPLETED for report_type={self.report_type} ===")
        return report
    ...
    def get_token_usage(self) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with token usage information:
            {
                ...
                "total_tokens": int,
                "call_count": int,
                "total_prompt_tokens": int,  # For backwards compatibility
                "total_completion_tokens": int,  # For backwards compatibility
            }
        """
        return self.token_tracker.summary()
    
    def reset_token_tracker(self) -> None:
        """Reset token usage tracker for a new report run."""
