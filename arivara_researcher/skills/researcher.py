...
class ResearchConductor:
    ...
    async def conduct_research(self):
        ...
        # Send initial progress message to show research has started
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "research_started",
                f"🔬 Research phase started. Gathering information for: {self.researcher.query[:100]}...",
                self.researcher.websocket,
            )
        ...
        if self.researcher.verbose:
            ...
            await stream_output(
                ...
                f"✅ Research phase completed! Gathered {context_size:,} characters of context.\n💸 Total Research Costs: ${self.researcher.get_costs()}\n\n📝 Starting report generation...",
                self.researcher.websocket,
            )
            if self.json_handler:
    ...
    async def _get_context_by_web_search(self, query, scraped_data: list | None = None, query_domains: list | None = None):
        ...
        # Process sub-queries with progress updates
        total_sub_queries = len(sub_queries)
        if self.researcher.verbose:
            await stream_output(
                "logs",
                "processing_subqueries",
                f"🔄 Processing {total_sub_queries} research queries...",
                self.researcher.websocket,
            )
        ...
        try:
            ...
            async def process_with_progress(sub_query, index):
                if self.researcher.verbose and index < total_sub_queries:
                    await stream_output(
                        "logs",
                        "subquery_progress",
                        f"📊 Processing query {index + 1}/{total_sub_queries}: {sub_query[:50]}...",
                        self.researcher.websocket,
                    )
            ...
            context = await asyncio.gather(
                *[
                    process_with_progress(sub_query, idx)
            ...
            if self.researcher.verbose:
                successful_queries = sum(1 for c in context if c)
                await stream_output(
                    "logs",
                    "subqueries_complete",
                    f"✅ Completed research for {successful_queries}/{total_sub_queries} queries. Combining context...",
                    self.researcher.websocket,
                )
            ...
            if self.researcher.verbose:
                await stream_output(
                    "logs",
                    "research_error",
                    f"❌ Error during research: {str(e)[:100]}",
                    self.researcher.websocket,
                )
            return []
