...
from arivara_researcher.utils.logging_config import setup_research_logging
from arivara_researcher.utils.enum import Tone
...
# Import chat_module router for user-memory chatbot
try:
    # Add chat_module to Python path so its internal 'app.' imports work
    # This allows chat_module files to use 'from app.xxx import ...' syntax
    ...
    # Now import using chat_module.app path
    from chat_module.app.routers import chat_router, health_router
    CHAT_MODULE_AVAILABLE = True
except ImportError as e:
    logger.warning(f"Chat module not available: {e}. User-memory chatbot endpoints will not be available.")
...
@app.get("/pdf/{research_id}")
async def read_pdf(request: Request, research_id: str):
    pdf_path = os.path.join('outputs', f"{research_id}.pdf")
    if not os.path.exists(pdf_path):
        return {"message": "PDF not found."}
    return FileResponse(pdf_path, media_type="application/pdf")

@app.get("/docx/{research_id}")
async def read_docx(request: Request, research_id: str):
    docx_path = os.path.join('outputs', f"{research_id}.docx")
    if not os.path.exists(docx_path):
        return {"message": "DOCX not found."}
...
# Mount chat_module routers for user-memory chatbot
# These endpoints use the same Supabase database and authentication as the main backend
if CHAT_MODULE_AVAILABLE:
    app.include_router(health_router, prefix="/api", tags=["Health"])
    app.include_router(chat_router, prefix="/api", tags=["Chat"])
    logger.info("Chat module integrated: User-memory chatbot endpoints available at /api/chat")
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        # Try new authenticated handler first
        from backend.server.websocket_handler import AuthenticatedWebSocketHandler
        handler = AuthenticatedWebSocketHandler(manager)
        ...
        from backend.auth.auth_middleware import remove_websocket_user
